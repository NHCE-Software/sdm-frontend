<script>
  ...
  import { addModalOpen, baseurl } from "../store/store";
  ...
  $: {
    if (newData) {
      ...
      let cp = parseInt(newData.grade.marks.computer || "0");
      let e = parseInt(newData.grade.marks.electronics || "0");
      let o = parseInt(newData.grade.marks.others || "0");
      let b = parseInt(newData.grade.marks.bio || "0");
      temppcmscore = (m + c + p + cp + e + o + b) / 3;
    }
  }
</script>
    ...
    <div class="modal-box relative flex flex-col max-w-5xl h-5/6">
        ...
        <div class="py-3 flex flex-col">
          ...
          <div class="my-4 grid grid-cols-3 gap-3">
            ...
            <Input
              ...
              bind:value={newData.internal.enqno}
            />
            <Input
              type="date"
      ...
      {#if mode === "grade"}
        ...
        <div class="grid grid-cols-3 gap-3">
          ...
          <Input
            header="Chemistry"
            bind:value={newData.grade.marks.chemistry}
          />
          ...
          />
          <Input header="Others" bind:value={newData.grade.marks.others} />
        </div>
      ...
      {#if mode === "remarks"}
        <div class="py-3">
          <div class=" gap-3 grid  py-3">
            <Input header="Remark" bind:value={newData.remark} />
            <Input
              type="area"
              header="Notes"
              rows={12}
      ...
      <button
        on:click={async () => {
          ...
          console.log(newData);
          const body = {
            query: `mutation Mutation($record: CreateOneStudentInput!) {
                    leadCreateOne(record: $record) {
                      recordId
                    }
                  }`,
            variables: {
              record: {
                ...newData,
                grade: { ...newData.grade, pcmscore: temppcmscore },
              },
            },
          };
          ...
          if (res.data.data)
            swal("Insert Successful", "Changes have be saved", "success");
          else
            swal(
              "Insert Unsuccessful",
